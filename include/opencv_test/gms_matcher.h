#pragma once
#include "opencv_test/Header.h"

#define THRESH_FACTOR 3
...
inline Mat DrawInlier(Mat &src1, Mat &src2, vector<KeyPoint> &kpt1, vector<KeyPoint> &kpt2, vector<DMatch> &inlier, int type) {
	...
	Mat temp_src1(height, width, CV_8UC3, Scalar(0, 0, 0));
	Mat temp_src2(height, width, CV_8UC3, Scalar(0, 0, 0));

	cv::cvtColor(src1, temp_src1, cv::COLOR_GRAY2BGR);
	cv::cvtColor(src2, temp_src2, cv::COLOR_GRAY2BGR);
	temp_src1.copyTo(output(Rect(0, 0, src1.cols, src1.rows)));
	temp_src2.copyTo(output(Rect(src1.cols, 0, src2.cols, src2.rows)));
